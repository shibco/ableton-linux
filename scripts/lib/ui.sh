...
declare -A UI_TEXT=(
    ...
    [u_plan_heading]='Uninstall Ableton Linux'
    [u_plan_remove_runtimes]='Remove the installed Wine runtime and saved previous versions: %s'
    [u_plan_leave_desktop]='Leave desktop shortcuts, file-opening settings, and older Wine runtimes unchanged'
    [u_plan_remove_desktop]='Remove Ableton Linux launchers, icons, file-opening support, and diagnostic tools'
    [u_plan_restore_mime]='Restore the file-opening defaults that were in use before installation'
    [u_plan_clear_mime]='Clear only the file-opening defaults that name Ableton Linux'
    [u_plan_leave_mime]='Leave file-opening defaults unchanged'
    [u_plan_remove_settings]='Remove installer settings and support files'
    [u_plan_unregister_pipeasio]='Remove PipeASIO from the kept Wine prefix: %s'
    [u_plan_delete_prefix]='Delete the Wine prefix, including Live and its authorisation: %s'
    [u_q_delete_prefix]='Delete the Wine prefix? This removes Live and its Wine-side authorisation'
    [u_q_stop_clients]='Stop every running program in the selected prefix and uninstall?'
    [u_stop_services]='Stop Ableton Linux background services'
    [u_unregister_pipeasio]='Remove PipeASIO from the kept Wine prefix'
    [u_kept_restored_launcher]='Kept the restored launcher %s'
    [u_restored_earlier_launcher]='Restored the earlier launcher %s'
    [u_kept_restored_file]='Kept your already-restored earlier file at %s'
    ...
    [u_remove_runtime_files]='Remove the Wine runtime, launchers, and support files'
    [u_kept_prefix]='Kept the Wine prefix %s'
    [u_no_prefix]='No Wine prefix to remove at %s'
    [u_kept_backups]='kept manual installer backups at %s'
    [u_usage_line1]='Usage: uninstall.sh [--keep-prefix|--delete-prefix] [--yes] [--dry-run]'
    [u_usage_line2]='The prefix is kept by default. --delete-prefix removes Live and its Wine-side authorisation after validating a project marker (or the legacy default path).'
    [u_prefix_flag_deprecated]='--prefix is deprecated; use --delete-prefix'
    [u_q_delete_yes]='[D]elete the Wine prefix'
    [u_q_delete_no]='[K]eep the Wine prefix'
    [u_retry_installer]='Retry: %s uninstall %s --yes'
    [u_retry_script]='Retry: bash %s %s --yes'
    [u_done_with_warnings]='runtime and requested prefix removal completed; the warnings above describe optional items that remain'
    [u_mime_prestate_unsafe]='File-opening defaults were left unchanged because the saved settings at %s could not be read safely'
    [u_mime_prestate_unsafe_hint]='Move that object aside only after checking that it is not your file.'
    [u_mime_prestate_unrecognised]='File-opening defaults were left unchanged because the saved settings at %s are not recognised'
    [u_mime_prestate_unrecognised_hint]='Move that file aside only if you no longer need those saved settings.'
    [u_state_overlaps_core]='Ableton Linux support files were not cleaned separately because their path overlaps the Wine runtime or prefix: %s'
    [u_state_overlaps_core_hint]='The requested Wine removal will still continue; inspect any remaining support path afterward.'
    [u_state_unrecognised]='Ableton Linux support files were left unchanged because their directory is not recognised: %s'
    [u_state_unrecognised_hint]='Inspect that directory and move it aside only if you are certain it belongs to this project.'
    [u_mime_prestate_unreadable]='File-opening defaults were left unchanged because the saved settings at %s could not be read'
    [u_check_file_permissions_hint]='Check the file permissions before retrying.'
    [u_inventory_untrusted]='Desktop shortcuts, file-opening settings, and older Wine runtimes were left unchanged because the installer'\''s file list could not be trusted'
    [u_inventory_untrusted_hint]='Inspect %s and %s, then retry after moving aside only a file you know is damaged.'
    [u_plan_link_failed]='the Link shutdown plan could not be produced; Link settings may remain enabled'
    [u_plan_link_failed_hint]='Close any Ableton Link helper before retrying.'
    [u_old_runtime_in_use]='an older Wine runtime was left unchanged because PID %s is using it: %s'
    [u_old_runtime_in_use_hint]='Close that program before retrying if you want this older runtime removed.'
    [u_mime_prestate_reread_failed]='File-opening defaults were left unchanged because the saved settings at %s could not be read again'
    [u_mime_prestate_changed]='File-opening defaults were left unchanged because their saved settings changed while uninstall was starting: %s'
    [u_mime_prestate_changed_hint]='Review that file, then retry if you still want the old defaults restored.'
    [u_state_changed]='Ableton Linux support files were left unchanged because their directory changed while uninstall was starting: %s'
    [u_inspect_directory_hint]='Inspect that directory before retrying.'
    [u_old_runtime_unrecognised]='an older Wine runtime was left unchanged because the installer no longer recognises it: %s'
    [u_inspect_runtime_hint]='Inspect that runtime before retrying.'
    [u_legacy_state_unprepared]='legacy shortcut support files were left unchanged because their directory could not be prepared safely'
    [u_inspect_path_hint]='Inspect %s before retrying.'
    [u_old_runtime_in_use_late]='an older Wine runtime was left unchanged because PID %s began using it: %s'
    [u_adoption_tmp_remains]='temporary legacy-install cleanup remains at %s'
    [u_adoption_tmp_remains_hint]='Check its permissions; the recognised installation is already safe to retry.'
    [u_adoption_tmp_active]='temporary legacy-install cleanup remains active at %s'
    [u_adoption_tmp_active_hint]='Keep those files for inspection before retrying.'
    [u_rollback_scan_tmp_failed]='older saved Wine runtimes could not be inspected because a temporary uninstall file could not be created'
    [u_rollback_scan_tmp_failed_hint]='Remove any older saved runtimes manually, or retry after checking the temporary-directory permissions.'
    [u_rollback_scan_path_failed]='older saved Wine runtimes related to %s could not be located'
    [u_rollback_scan_path_failed_hint]='Retry after checking that the standard path tools are working.'
    [u_rollback_parent_unsafe]='older saved Wine runtimes were left unchanged because their parent directory is not safe to inspect: %s'
    [u_inspect_that_path_hint]='Inspect that path before retrying.'
    [u_rollback_scan_tmp_unwritable]='older saved Wine runtimes could not be inspected because the temporary uninstall file is not writable: %s'
    [u_rollback_scan_tmp_unwritable_hint]='Check that file and the temporary-directory permissions before retrying.'
    [u_rollback_parent_unreadable]='older saved Wine runtimes were left unchanged because %s could not be inspected'
    [u_check_directory_permissions_hint]='Check the directory permissions before retrying.'
    [u_rollback_unidentified]='an older saved Wine runtime could not be identified and was left unchanged: %s'
    [u_rollback_unrecognised]='an unrecognised saved Wine runtime was left unchanged at %s'
    [u_rollback_unrecognised_hint]='Inspect it and remove it manually only if you are certain it belongs to this project.'
    [u_scan_tmp_remains]='a temporary uninstall file remains at %s'
    [u_tmp_remains_hint]='Check its permissions, then remove it or retry uninstall.'
    [u_link_still_enabled]='Ableton Link integration may still be enabled'
    [u_link_still_enabled_hint]='Close any Link helper and check the user service before retrying.'
    [u_shortcuts_helper_unusable]='GNOME shortcut restoration was skipped because its project helper could not be used: %s'
    [u_shortcuts_helper_unusable_hint]='Keep the saved shortcut values, repair or reinstall that helper, then retry.'
    [u_shortcuts_unfinished]='GNOME shortcut restoration did not finish; saved values remain under %s'
    [u_shortcuts_unfinished_hint]='Close Live and make sure gsettings is available before retrying.'
    [u_shortcuts_incomplete]='GNOME shortcut restoration is incomplete; saved values remain at %s'
    [u_shortcuts_incomplete_hint]='Close Live and make sure gsettings can update your desktop shortcuts.'
    [u_launcher_restored_copy_remains]='the earlier launcher is restored at %s, but its extra saved copy remains at %s'
    [u_check_saved_copy_permissions_hint]='Check the saved copy'\''s permissions before retrying.'
    [u_kept_restored_launcher_changed_copy]='kept the restored launcher at %s and a changed saved copy at %s'
    [u_kept_restored_launcher_changed_copy_hint]='Review the changed saved copy before moving or deleting it.'
    [u_kept_modified_launcher]='kept the user-modified launcher at %s and its saved earlier launcher at %s'
    [u_kept_modified_launcher_hint]='Review both files and move your preferred launcher into place before retrying.'
    [u_launcher_saved_copy_invalid]='the project launcher at %s was left unchanged because its saved earlier launcher is missing or changed: %s'
    [u_launcher_saved_copy_invalid_hint]='Restore or review the saved launcher before retrying.'
    [u_launcher_restore_failed]='the earlier launcher at %s could not be restored at %s; both files were kept'
    [u_check_destination_permissions_hint]='Check the destination permissions before retrying.'
    [u_launcher_restored_copy_remains_late]='the earlier launcher was restored at %s, but its extra saved copy remains at %s'
    [u_kept_changed_saved_launcher]='kept a changed saved launcher at %s and its earlier copy at %s'
    [u_kept_changed_saved_launcher_hint]='Review both files and keep the one you want before retrying.'
    [u_file_restore_failed]='the earlier file was not restored at %s; its saved copy remains at %s'
    [u_file_replace_failed]='the Ableton Linux file at %s could not be replaced by its saved earlier copy at %s'
    [u_file_replace_failed_hint]='Check both paths and their permissions before retrying.'
    [u_file_remains]='an Ableton Linux file remains at %s'
    [u_kept_modified_config]='kept user-modified configuration at %s'
    [u_kept_modified_config_hint]='Move or delete it yourself only if you no longer need those settings.'
    [u_kept_changed_link]='kept a link at %s because it was changed or points somewhere else'
    [u_kept_changed_link_hint]='Remove it yourself only if you recognise it as obsolete.'
    [u_kept_custom_linkd]='kept a modified custom Ableton Link helper at %s'
    [u_kept_custom_linkd_hint]='Remove it yourself only if you no longer use that custom Link helper.'
    [u_kept_unrecognised_file]='kept an unrecognised or user-modified file at %s'
    [u_move_aside_if_certain_hint]='Move it aside only if you are certain it belongs to this project.'
    ...
    [u_mime_tmp_remains]='a temporary file-opening settings file remains at %s'
    [u_mime_xdg_missing]='some Ableton file-opening defaults could not be cleared because the xdg-utils tool is missing'
    [u_mime_xdg_missing_hint]='Install the xdg-utils package before retrying.'
    [u_mime_query_failed]='the current app for %s could not be checked and may still point to Ableton Linux'
    [u_mime_query_failed_hint]='Install xdg-utils or check that it can read your desktop settings, then retry.'
    [u_mime_file_unreadable]='the setting for %s was left unchanged because %s could not be read'
    [u_mime_still_default]='Ableton Linux is still set to open %s in %s'
    [u_mime_still_default_hint]='Remove that default or make the file writable before retrying.'
    [u_mime_verify_failed]='the setting for %s could not be verified in %s'
    [u_mime_query_failed_after]='the current app for %s could not be checked after Ableton Linux files were removed'
    [u_mime_restore_failed]='the previous app for %s could not be restored'
    [u_mime_restore_failed_hint]='Check that %s is writable, then retry uninstall.'
    [u_mime_prestate_changed_late]='File-opening defaults were left unchanged because their saved settings changed before they could be used: %s'
    [u_panel_shortcuts_unconfirmed]='PipeASIO Settings shortcuts were left unchanged because the installer could not confirm that it created them'
    [u_panel_shortcuts_unconfirmed_hint]='Inspect those shortcuts and retry after repairing the installer'\''s file list.'
    [u_files_unconfirmed]='some Ableton Linux launchers or support files were left unchanged because the installer could not confirm that it created them'
    [u_files_unconfirmed_hint]='Inspect the remaining files and retry after repairing the installer'\''s file list.'
    [u_old_runtime_location_unrecognised]='an older Wine runtime was left unchanged because its location is no longer recognised: %s'
    [u_rollback_target_unrecognised]='an older saved Wine runtime was left unchanged because its target is no longer recognised: %s'
    [u_old_runtime_location_changed]='an older Wine runtime was left unchanged because its location changed before removal: %s'
    [u_old_runtime_remains]='an older Wine runtime remains at %s'
    [u_rollback_target_changed]='an older saved Wine runtime was left unchanged because its target changed before removal: %s'
    [u_rollback_remains]='an older saved Wine runtime remains at %s'
    [u_mime_cache_not_refreshed]='the desktop file-type cache was not refreshed under %s'
    [u_mime_cache_not_refreshed_hint]='Install shared-mime-info, then rerun uninstall or update-mime-database for that directory.'
    [u_mime_cache_refresh_failed]='the desktop file-type cache could not be refreshed under %s'
    [u_mime_cache_refresh_failed_hint]='Check the directory permissions, then rerun uninstall or update-mime-database for that directory.'
    [u_desktop_cache_not_refreshed]='the desktop application cache was not refreshed under %s'
    [u_desktop_cache_not_refreshed_hint]='Install desktop-file-utils, then rerun uninstall or update-desktop-database for that directory.'
    [u_desktop_cache_refresh_failed]='the desktop application cache could not be refreshed under %s'
    [u_desktop_cache_refresh_failed_hint]='Check the directory permissions, then rerun uninstall or update-desktop-database for that directory.'
    [u_cache_dir_remains]='the Ableton Linux cache directory remains at %s'
    [u_cache_dir_remains_hint]='Remove it after checking that it contains no files you need.'
    [u_cache_path_unsafe]='the project cache path was left unchanged because it is not a safe empty directory: %s'
    [u_inspect_path_remove_if_recognised_hint]='Inspect that path and remove it manually only if you recognise it.'
    [u_mime_prestate_remains]='saved file-opening settings remain at %s'
    [u_manifest_remains]='the Ableton Linux file list remains at %s'
    [u_settings_remain]='installer settings remain at %s'
    [u_settings_changed]='installer settings at %s changed during uninstall and were preserved'
    [u_settings_changed_hint]='Inspect them and remove them manually only if you recognise them as Ableton Linux settings.'
    [u_state_unsafe_remove]='Ableton Linux support files were left unchanged because their directory is not safe to remove: %s'
    [u_state_unconfirmed]='Ableton Linux support files were left unchanged because the installer could not confirm that it created their directory: %s'
    [u_state_check_failed]='Ableton Linux support files could not be fully checked after cleanup at %s'
    [u_state_check_failed_hint]='Check the directory permissions and contents before retrying.'
    [u_state_files_remain]='Ableton Linux support files remain at %s'
    [u_state_files_remain_hint]='Check the remaining files and directory permissions before retrying.'
    [u_state_changed_cleanup]='the Ableton Linux support directory changed during cleanup and was kept at %s'
    [u_state_marker_remains]='the final Ableton Linux support record remains at %s'
    [u_state_dir_empty_remains]='the empty Ableton Linux support directory remains at %s'
    [u_state_dir_empty_remains_hint]='Check the parent-directory permissions before retrying.'
    [u_state_path_empty_unprepared]='an empty Ableton Linux support path remains at %s and could not be prepared for an automatic retry'
    [u_state_path_empty_unprepared_hint]='Inspect it, then remove that empty path manually.'
