...
19001. `scripts/bench-osc.py` speaks both directions from the shell, and
`scripts/bench-run.sh` fills its DSP column from the device's reports.
...
- Harness integration: `bench-run.sh` averages `/abl/bench/cpu` over its
  sampling window into `dsp_load_pct` whenever no operator value is given.
  Planned next: `bench-workload.sh` using `/abl/bench/ready` as the
  set-loaded signal and driving playback scenarios with `/abl/bench/play`.
