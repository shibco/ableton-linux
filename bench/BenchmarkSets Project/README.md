...
Use the Live sets to compare CPU use and audio performance. The
[benchmark guide](../README.md) explains the full process.

Run the suite commands from the repository root. Start with Live closed.
...
| `Benchmark_Empty.als` | the control device | measures the active audio session and control device |
...
```bash
for f in bench/BenchmarkSets\ Project/*.als; do
  echo "$f: $(zcat "$f" | grep -c abl-bench)"
done
```
...
the idle set. The other files in the folder belong to `Benchmark_Empty.als`.
...
Use the following Windows VST3 files:
