...
```bash
scripts/bench-workload.sh baseline --set "bench/BenchmarkSets Project/Benchmark_Empty.als" -n 3
```
...
for Live device loaded, so the runtime never boots. Runs on that set are
operator-driven, and `bench-run.sh` needs the DSP reading passed on the
command line. Everything else in the folder is the same set as
`Benchmark_Empty.als`.
