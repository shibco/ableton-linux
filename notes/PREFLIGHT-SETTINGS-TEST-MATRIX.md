...
| PREF-REMOVE | Uninstall removes only an exact valid unchanged record | `test-preflight-preferences.sh`: valid removal; changed and malformed preservation; integration static call check |
...
| UNINSTALL-OWNERSHIP | Mutable preferences are separate data and actual uninstall removes only a valid unchanged object after all requested core removal succeeds | `test-uninstall-boundary.sh`: keep-prefix and delete-prefix removal-order logs; valid, malformed, future, symlink, directory and valid-to-valid race cases; runtime/prefix failures preserve preferences |
...
| PACKAGE-MANIFEST | Support library is an authoritative managed file; mutable preferences are separate | `test-preflight-integration.sh`: manifest/uninstall surface and checked-removal API |
